...
function LeaderboardRow({ entry, isCreator = false }: { entry: any; isCreator?: boolean }) {
    ...
    return (
        <div style={{
            ...
            padding: '14px 24px', borderBottom: '1px solid var(--dash-border)', transition: 'background 0.12s',
        }}>
            <div style={{
                width: '32px', height: '32px', borderRadius: '8px', flexShrink: 0,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '12px', fontWeight: 700, background: rankColor.bg, color: rankColor.color,
            }}>
                {rank === 1 ? <Trophy style={{ width: '14px', height: '14px' }} /> :
                 rank === 2 ? <Medal style={{ width: '14px', height: '14px' }} /> :
                 rank === 3 ? <Medal style={{ width: '14px', height: '14px' }} /> : `#${rank}`}
            </div>
